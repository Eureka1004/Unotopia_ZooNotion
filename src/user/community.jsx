...
const eventData = {
  ...
  year: '2025',
  description:
    "The Jakarta Pet Expo (JPE) will held its second edition on 26-29 November, 2025, at the Jakarta International Expo (JI Expo) Kemayoran in Jakarta, Indonesia. This event is set to emerge as the leading international trade expo for pet care products in Indonesia.",
  imageUrl:
...
function EventCard({ title, imageUrl, imageAlt, organizer, year, description }) {
  return (
    <article className="bg-green-900 text-white rounded-lg p-6 flex flex-col shadow-lg hover:shadow-2xl transition-shadow duration-300 max-w-sm mx-auto">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="border border-green-500 rounded-full px-3 py-1 text-xs whitespace-nowrap text-green-300">
          Diselenggara oleh: {organizer}
        </div>
        <div className="border border-green-500 rounded-full px-3 py-1 text-xs whitespace-nowrap text-green-300">
          Tahun Event: {year}
        </div>
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="max-w-7xl mx-auto mt-4 px-4 sm:px-6 lg:px-8">
        <img
          src="https://storage.googleapis.com/workspace-0f70711f-8b4e-4d94-86f1-2a93ccde5887/image/1e4bd7bf-b9ce-4d00-80a8-d3a8310d326c.png"
          alt="Illustrative banner showing a safari adventure with various animal silhouettes on wood background"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">{cards}</div>
      </main>
    </div>
  );
}
